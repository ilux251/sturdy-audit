...
* [`src/sturdy_audit/`](src/sturdy_audit/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/sturdy_audit/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `sturdy-audit` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/sturdy_audit/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns sturdy-audit.example
  (:require [sturdy-audit.config :as config])
